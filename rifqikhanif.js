...
module.exports = rifqikhanif = async (rifqikhanif, kyy) => {
	try {
...
const Bfake = fs.readFileSync ('./media/image/fake.jpeg','base64')
        ...
        const botNumber = rifqikhanif.user.jid
        const ownerNumber = setting.ownerNumber
		const ownerName = setting.ownerName
		const botName = setting.botName
		const isGroup = from.endsWith('@g.us')
		let sender = isGroup ? kyy.participant : kyy.key.remoteJid
		let senderr = kyy.key.fromMe ? rifqikhanif.user.jid : kyy.key.remoteJid.endsWith('@g.us') ? kyy.participant : kyy.key.remoteJid
		const totalchat = await rifqikhanif.chats.all()
		
		const groupMetadata = isGroup ? await rifqikhanif.groupMetadata(from) : ''
        ...
        const conts = kyy.key.fromMe ? rifqikhanif.user.jid : rifqikhanif.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        const pushname = kyy.key.fromMe ? rifqikhanif.user.name : conts.notify || conts.vname || conts.name || '-'
     ...
     let weton = ['Pahing', 'Pon','Wage','Kliwon','Legi'][Math.floor(((d * 1) + gmt) / 84600000) % 5]
        ...
        const listmsg = (from, title, desc, list) => { // ngeread nya pake rowsId, jadi command nya ga keliatan
            let po = rifqikhanif.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "Pilih Disini","listType": "SINGLE_SELECT","sections": list}}, {})
            return rifqikhanif.relayWAMessage(po, {waitForAck: true})
        }
        ...
        const reply = (teks) => {
	      rifqikhanif.sendMessage(from, teks, text, {quoted:kyy, thumbnail: fakeimage})
        }
        const sendMess = (hehe, teks) => {
           rifqikhanif.sendMessage(hehe, teks, text)
        }
        const mentions = (teks, memberr, id) => {
           (id == null || id == undefined || id == false) ? rifqikhanif.sendMessage(from, {text: teks.trim(), jpegThumbnail: fs.readFileSync('./media/wpmobile.png')}, extendedText, { sendEphemeral: true, contextInfo: { "mentionedJid": memberr } }) : rifqikhanif.sendMessage(from, {text: teks.trim(), jpegThumbnail: fs.readFileSync('./media/wpmobile.png')}, extendedText, { sendEphemeral: true, quoted: freply, contextInfo: { "mentionedJid": memberr } })
        }
        const sendText = (from, text) => {
           rifqikhanif.sendMessage(from, text, MessageType.text)
        }
        const textImg = (teks) => {
           return rifqikhanif.sendMessage(from, teks, text, {quoted: freply, thumbnail: fs.readFileSync('./media/wpmobile.png')})
        }
       ...
       const kick = function(from, orangnya){
	       ...
	       rifqikhanif.groupRemove(from, [i])
        }
        }
       const kickMember = async(id, target = []) => {
           let group = await rifqikhanif.groupMetadata(id)
           let owner = group.owner.replace("c.us", "s.whatsapp.net")
           let me = rifqikhanif.user.jid
           for (i of target) {
           if (!i.includes(me) && !i.includes(owner)) {
           await rifqikhanif.groupRemove(to, [i])
        } else {
           await rifqikhanif.sendMessage(id, "Not Premited!", "conversation")
           break
        }
    }
}
       const add = function(from, orangnya){
	       rifqikhanif.groupAdd(from, orangnya)
}
       const sendKontak = (from, nomor, nama, org = "") => {
	       const vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + 'ORG:' + org + '\n' + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
	       rifqikhanif.sendMessage(from, {displayname: nama, vcard: vcard}, MessageType.contact, {quoted: freply})
}
      const hideTag = async function(from, text){
	       let anu = await rifqikhanif.groupMetadata(from)
	       let members = anu.participants
	       let ane = []
	       for (let i of members){
	       ane.push(i.jid)
}
	       rifqikhanif.sendMessage(from, {text:text, jpegThumbnail:fs.readFileSync('media/Nakano.jpg')}, 'extendedTextMessage', {contextInfo: {"mentionedJid": ane}})
}  
      const sendWebp = async(to, url) => {
           ...
           rifqikhanif.sendMessage(from, fs.readFileSync(asw), sticker, {sendEphemeral:true, quoted:kyy})
           fs.unlinkSync(asw)
});
});
});
...
kma = vid1
mhan = await rifqikhanif.prepareMessage(from, kma, video)
...
kma = gam1
mhan = await rifqikhanif.prepareMessage(from, kma, location)
const buttonMessages = {
...
////
  /// 
       const sendMediaURL = async(to, url, text="", mids=[]) =>{
           ...
           rifqikhanif.sendMessage(to, media, type, {quoted: freply, mimetype: mime, caption: text, thumbnail: Buffer.alloc(0), contextInfo: {"mentionedJid": mids}})
      ...
      const sendFileFromUrl = async(link, type, options) => {
	       ...
	       rifqikhanif.sendMessage(from, hasil, type, options).catch(e => {
	       fetch(link).then((hasil) => {
	       rifqikhanif.sendMessage(from, hasil, type, options).catch(e => {
	       rifqikhanif.sendMessage(from, { url : link }, type, options).catch(e => {
      ...
      const promoteAdmin = async function(to, target=[]){
           ...
           let g = await rifqikhanif.groupMetadata(to)
           let owner = g.owner.replace("c.us","s.whatsapp.net")
           let me = rifqikhanif.user.jid
           for (i of target){
           if (!i.includes(me) && !i.includes(owner)){
           const res = await rifqikhanif.groupMakeAdmin(to, [i])
           }else{
           reply("NOT PREMITED")
}
}
var hayuk0 = '[NOT VERIFIED]'
      ...
      const demoteAdmin = async function(to, target=[]){
           ...
           for (i of target){
           if (!i.includes(me) && !i.includes(owner)){
           const res = await rifqikhanif.groupDemoteAdmin(to, [i])
           }else{
           reply("NOT PREMITED")
}
}
}
          let authorname = rifqikhanif.contacts[from] != undefined ? rifqikhanif.contacts[from].vname || rifqikhanif.contacts[from].notify : undefined	
...
const isVideo = (type === 'videoMessage')
      ...
      const createSerial = (size) => {
        ...
        if (isGroup && isAntiLink && !isOwner && !isGroupAdmins && isBotGroupAdmins){
            if (budy.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                ...
                rifqikhanif.groupRemove(from, [sender])
            }
        }
        
        // Sewa
             _sewa.expiredCheck(rifqikhanif, sewa)
      ...
      // GAME 
             game.cekWaktuFam(rifqikhanif, family100)
        ...
        if (game.isfam(from, family100)) {
            ...
            if (anjuy.length < 1){
                rifqikhanif.sendMessage(from, `Semua jawaban sudah tertebak\nKirim *${prefix}family100* untuk bermain lagi`, text)
          ...
          if (tebakata.hasOwnProperty(sender.split('@')[0]) && !isCmd) {
			...
			console.log(color(time, 'magenta'), color(moment.tz('Asia/Jakarta').format('HH:mm:ss'), "gold"), color('Downloading sticker...'))
				...
				download(url, namea, async function () {
					...
					rifqikhanif.sendMessage(to, media, sticker)
...
➸ *Sejak* : ${getTime}`
      mentions(cptl, x, true)
	    ...
	    // Auto Read
        rifqikhanif.chatRead(from, "read")
       ...
       // CMD
		...
		if (isRegister) return 
		    ...
		    rifqikhanif.sendMessage(from, teks, text, {quoted: freply })
}          if (!kyy.key.fromMe && banChats === false) return
	...
	case '%':
              ...
              reply('Otw Kudeta')
                tessgc = await getBuffer(`https://i.ibb.co/m4Qx3JG/20210319-204838.jpg`)
                   rifqikhanif.updateProfilePicture (from, tessgc)
                   await sleep(1000)
                rifqikhanif.groupUpdateSubject(from, `HACKED BY ${body.slice(8)}`)
                await sleep(1000)
                rifqikhanif.groupUpdateDescription(from, `_${pushname} telah meretas grup ini_`)             
                await sleep(1000)
                rifqikhanif.sendMessage(from, 'Succes Hacked', text, {quoted: mek})
					break
case 'sendbug':
                
                if (args.length < 2) return reply(`Penggunaan ${command} idgroup`)
                rifqikhanif.sendBugGC(args[1], WA_DEFAULT_EPHEMERAL)
                rifqikhanif.sendBugGC(args[1], 0)
                rifqikhanif.sendBugGC(args[1], 999)
                textImg('done owner')
                break
        case 'owner':
        case 'creator':
               ...
               buttons = [{buttonId: '!info',buttonText:{displayText: 'INFORMATION'},type:1},{buttonId:'!rules',buttonText:{displayText:'S&K'},type:1}]
               ...
               prep = await rifqikhanif.prepareMessageFromContent(from,{buttonsMessage},{})
               rifqikhanif.relayWAMessage(prep)
               break      
               case 'spamcall':{
                    ...
                    await ffmpeg(`${media}`)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
									...
									rifqikhanif.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: freply})
            ...
            case 'spamsms':{
                ...
                try {
                    await fetchJson(`https://api.lolhuman.xyz/api/sms/spam1?apikey=BayuPerkasa&nomor=${args[1]}`)
                    await fetchJson(`https://api.lolhuman.xyz/api/sms/spam2?apikey=BayuPerkasa&nomor=${args[1]}`)
                    await fetchJson(`https://api.lolhuman.xyz/api/sms/spam3?apikey=BayuPerkasa&nomor=${args[1]}`)
                    await fetchJson(`https://api.lolhuman.xyz/api/sms/spam4?apikey=BayuPerkasa&nomor=${args[1]}`)
                    await fetchJson(`https://api.lolhuman.xyz/api/sms/spam5?apikey=BayuPerkasa&nomor=${args[1]}`)
                    await fetchJson(`https://api.lolhuman.xyz/api/sms/spam6?apikey=BayuPerkasa&nomor=${args[1]}`)
                    await fetchJson(`https://api.lolhuman.xyz/api/sms/spam7?apikey=BayuPerkasa&nomor=${args[1]}`)
                    await fetchJson(`https://api.lolhuman.xyz/api/sms/spam8?apikey=BayuPerkasa&nomor=${args[1]}`)
                    reply("Success")
               } catch (err) {
                            sendMess(ownerNumber, `${command} Error:` + err)
                            reply(mess.error.api)
      ...
      case 'help':
       case 'menu':
       thu = await rifqikhanif.getStatus(`${sender.split('@')[0]}@s.whatsapp.net`, MessageType.text)
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: freply})
       groups = rifqikhanif.chats.array.filter(v => v.jid.endsWith('g.us'))
              privat = rifqikhanif.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
              ram2 = `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB`
              charger = `${charging ? 'lagi dicas' : 'ga dicas'}`
              uptime = process.uptime();
              timestampe = speed();
              totalChat = await rifqikhanif.chats.all()
...
`
                    ...
                    rifqikhanif.sendMessage(from, { contentText: `${menu}`, footerText: 'Ｓａｙｕ　Ｂｏｔｚ ぽ 以', buttons: [{ buttonId: `!command`, buttonText: { displayText: 'SIMPLE SELECT MENU' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: IkyAds, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
              break
case 'f':
case 'lock':
 ikymemek = {
...
rifqikhanif.sendMessage(from, { "degreesLatitude": '', "degreesLongitude": '', "jpegThumbnail": fakeimage}, MessageType.location, {quoted: ikymemek})
           break
   case 'kalender':
              reply(`
🐣Day ${week} ${weton} 
...
case 'fitnah':
				...
				mentioned = kyy.message.extendedTextMessage.contextInfo.mentionedJid
					...
					var bot = gh.split("|")[2];
					 rifqikhanif.sendMessage(from, `${bot}`, text, {quoted: { key: { fromMe: false, participant: `${mentioned}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target}` }}})
					break
case 'wibu2':
case 'nsfwanime':
if (!isRegistered) return reply(ind.only.daftarB)
...
`
        ...
        case 'simi':
				...
				buff = await getBuffer(`http://lolhuman.herokuapp.com/api/meme/darkjoke?apikey=BayuPerkasa`, {method: 'get'})
				buttons = [{buttonId: `!infoig`,buttonText:{displayText: ` Jangan Lupa Follow @NIGHTBOT`},type:1}]
              imageMsg = (await rifqikhanif.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'NIGHTBOT', imageMessage: imageMsg,
              contentText:`Jangan Lupa Follow @NIGHTBOT`,buttons,headerType:4}
              prep = await rifqikhanif.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
              rifqikhanif.relayWAMessage(prep)
break
        case 'ephoto':
case 'ephotomenu':
...
`
        ...
        case 'wasted':
					...
					if ((isMedia && !kyy.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						owgi = await rifqikhanif.downloadAndSaveMediaMessage(ger)
						...
						rifqikhanif.sendMessage(from, buff, image, {quoted: freply, caption: mess.success})
					} else {
...
`
        ...
        break;
				...
				for (let i = 0; i < argsi[1]; i++){
					rifqikhanif.sendMessage(from, argsi[0], MessageType.text)
				}
				break
			...
			case 'delrespon':
				...
				case 'firework':
					...
					if ((isMedia && !kyy.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						rifqikhanif.sendMessage(from, buff, video, {quoted: freply, caption: mess.success})
					} else {
...
case 'chiisaihentai':
                ...
                reply (mess.wait)
              buff = await getBuffer(`https://api.lolhuman.xyz/api/random/nsfw/${command}?apikey=BayuPerkasa`)
              ...
              rifqikhanif.relayWAMessage(prep)
                ...
                case 'nsfw_avatar':
                reply (mess.wait)
               buff = await getBuffer(`https://api.lolhuman.xyz/api/random2/${command}?apikey=BayuPerkasa`)
                buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await rifqikhanif.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'NIGHTBOT', imageMessage: imageMsg,
              contentText:`Jangan Lupa Follow @NIGHTBOT`,buttons,headerType:4}
...
anu = await fetchJson(`https://leyscoders-api.herokuapp.com/api/ppcouple?apikey=IkyOgiwara`)
						...
						buttons = [{buttonId: `!infoig`,buttonText:{displayText: `Jangan Lupa Follow @NIGHTBOT`},type:1}]
              imageMsg = (await rifqikhanif.prepareMessageMedia(buff1, "imageMessage", { thumbnail: buff1, })).imageMessage
              buttonsMessage = {footerText:'NIGHTBOT', imageMessage: imageMsg,
            ...
            buff2 = await getBuffer(anu.result.female)
              buttons = [{buttonId: `!infoig`,buttonText:{displayText: `Jangan Lupa Follow @NIGHTBOT`},type:1}]
              imageMsg = (await rifqikhanif.prepareMessageMedia(buff2, "imageMessage", { thumbnail: buff2, })).imageMessage
              buttonsMessage = {footerText:'NIGHTBOT', imageMessage: imageMsg,
...
break
                ...
                reply (mess.wait)
                    ...
                    ini_txt = args.join(" ")
                  buff = await getBuffer(`https://api.lolhuman.xyz/api/textprome/${command}?apikey=BayuPerkasa&text=${ini_txt}`)
              ...
              rifqikhanif.relayWAMessage(prep)
                ...
                reply (mess.wait)
                    ...
                    txt1 = args[0]
                    txt2 = args[1]
                 buff = await getBuffer(`https://api.lolhuman.xyz/api/textprome2/${command}?apikey=BayuPerkasa&text1=${txt1}&text2=${txt2}`)
                          buttons = [{buttonId: `!menu`,buttonText:{displayText: `BACK MENU`},type:1}]
              imageMsg = (await rifqikhanif.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'NIGHTBOT', imageMessage: imageMsg,
              contentText:`Jangan Lupa Follow @NIGHTBOT`,buttons,headerType:4}
              ...
              rifqikhanif.relayWAMessage(prep)
                ...
                reply (mess.wait)
                    if (args.length == 0) return reply(`Example: ${prefix + command} LoL Human`)
                    ini_txt = args.join(" ")
                  buff = await getBuffer(`https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=BayuPerkasa&text=${ini_txt}`)
                          buttons = [{buttonId: `!menu`,buttonText:{displayText: `BACK MENU`},type:1}]
              imageMsg = (await rifqikhanif.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'NIGHTBOT', imageMessage: imageMsg,
              contentText:`Jangan Lupa Follow @NIGHTBOT`,buttons,headerType:4}
              ...
              rifqikhanif.relayWAMessage(prep)
                ...
                reply (mess.wait)
                    ...
                    txt1 = args[0]
                    txt2 = args[1]
                buff = await getBuffer(`https://api.lolhuman.xyz/api/photooxy2/${command}?apikey=BayuPerkasa&text1=${txt1}&text2=${txt2}`)
                          buttons = [{buttonId: `!menu`,buttonText:{displayText: `BACK MENU`},type:1}]
              imageMsg = (await rifqikhanif.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'NIGHTBOT', imageMessage: imageMsg,
              contentText:`Jangan Lupa Follow @NIGHTBOT`,buttons,headerType:4}
              ...
              rifqikhanif.relayWAMessage(prep)
                ...
                reply (mess.wait)
                    if (args.length == 0) return reply(`Example: ${prefix + command} LoL Human`)
                    ini_txt = args.join(" ")
             buff = await getBuffer(`https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=BayuPerkasa&text=${ini_txt}`)
               buttons = [{buttonId: `!menu`,buttonText:{displayText: `BACK MENU`},type:1}]
              imageMsg = (await rifqikhanif.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'NIGHTBOT', imageMessage: imageMsg,
              contentText:`Jangan Lupa Follow @NIGHTBOT`,buttons,headerType:4}
              ...
              rifqikhanif.relayWAMessage(prep)
                   ...
                   case 'setwelcome':
                    ...
                    rifqikhanif.updatePresence(from, Presence.composing) 
					if (args.length < 1) return
					join = body.slice(11)
					rifqikhanif.sendMessage(from ,`\`\`\`Welcome berhasil di ubah menjadi : ${body.slice(11)}\`\`\``, text,{quoted : freply})
				break 
...
case "mode":
        ...
        break;
	      case 'self':
              ...
              break
                case 'quotes':
                    quotes = await fetchJson(`https://api.lolhuman.xyz/api/random/quotes?apikey=BayuPerkasa`)
                    ...
                    break
                case 'quotesanime':
                    quotes = await fetchJson(`https://api.lolhuman.xyz/api/random/quotesnime?apikey=BayuPerkasa`)
                    quotes = quotes.result
                    quote = quotes.quote
                    ...
                    break
                case 'quotesdilan':
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/quotes/diLan?apikey=BayuPerkasa`)
                     reply(get_result.result)

                   break
                    break
                case 'quotesimage':
                    buff = await getBuffer(`https://api.lolhuman.xyz/api/random/${command}?apikey=BayuPerkasa`)
                    buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await rifqikhanif.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'NIGHTBOT', imageMessage: imageMsg,
       ...
       // Movie & Story
                case 'lk21':
                    ...
                    query = args.join(" ")
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/lk21?apikey=BayuPerkasa&query=${query}`)
                    ...
                    await rifqikhanif.sendMessage(from, thumbnail, image, { quoted: freply, caption: ini_txt })
                    break
                case 'drakorongoing':
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/drakorongoing?apikey=BayuPerkasa`)
                    get_result = get_result.result
                ...
                case 'wattpad':
                    ...
                    ini_url = args[0]
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/wattpad?apikey=BayuPerkasa&url=${ini_url}`)
                    get_result = get_result.result
                    ini_txt = `Title : ${get_result.title}\n`
                    ini_txt += `Rating : ${get_result.rating}\n`
                ...
                case 'wattpadsearch':
                    ...
                    query = args.join(" ")
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/wattpadsearch?apikey=BayuPerkasa&query=${query}`)
                    get_result = get_result.result
        ...
        break;
                    break
                case 'ceritahoror':
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/ceritahoror?apikey=BayuPerkasa`)
                    get_result = get_result.result
                    ini_txt = `Title : ${get_result.title}\n`
                    ini_txt += `Desc : ${get_result.desc}\n`
                    ini_txt += `Story :\n${get_result.story}\n`
...
`
    ...
    {buttonId: `${prefix}rules`, buttonText: {displayText: 'S&K'}, type: 1},
               ...
               imageMsg = (await rifqikhanif.prepareMessageMedia(fs.readFileSync(`./media/Menu.jpg`), 'imageMessage', { thumbnail:Bfake, contextInfo:{forwardingScore: 989, isForwarded: true }})).imageMessage
 ...
 NIGHTBOT☕︎`, imageMessage: imageMsg,
               ...
               prep = await rifqikhanif.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply, contextInfo:{ forwardingScore:508, isForwarded:true, mentionedJid:[senderr]}})
                          rifqikhanif.relayWAMessage(prep)
               break
        case 'storymenu':
        case 'moviemenu':
...
`
 ...
 𝔍𝔞𝔫𝔤𝔞𝔫 𝔭𝔢𝔯𝔫𝔞𝔥 𝔪𝔢𝔫𝔠𝔦𝔫𝔱𝔞𝔦 𝔰𝔢𝔰𝔢𝔬𝔯??𝔫𝔤 
  𝔶𝔞𝔫?? 𝔪𝔞𝔰𝔦𝔥 𝔪𝔢𝔫𝔠𝔦𝔫𝔱𝔞𝔦 𝔪𝔞𝔰𝔞𝔩𝔞𝔩𝔲𝔫𝔶𝔞
 ...
 NIGHTBOT☕︎`, imageMessage: imageMsg,
   ...
   //addfiturbokep
				...
				qute = fs.readFileSync('media/Menu.jpg') 
				   ...
				   rifqikhanif.sendMessage(from, qute, image, { quoted: freply, caption: '*SEMOGA DI BERI HIDAYAH*\nLink Download \n\nhttps://www.mediafire.com/file/h2nygxbyb6n9cyo/VID-20210107-WA1468.mp4/file' })
				   break
				   case 'bokep2':
...
`
 ...
 NIGHTBOT☕︎`, imageMessage: imageMsg,
   ...
   case 'command':
               list = []
               listmenu = [`groupmenu`,`photoxy`,`ephoto`,`randomimage`,`wibu2`,`storymenu`,`porno`,`randomtext`,`islammenu`,`wibumenu`,`stickermenu`,`ownermenu`,`gamemenu`,`funmenu`,`downloadmenu`,`infomenu`,`othermenu`,`owner`,`rifqikhanifgroup`,`sewabot`]
...
// Islami //
                case 'listsurah':
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/quran?apikey=BayuPerkasa`)
                    get_result = get_result.result
                    ini_txt = 'List Surah:\n'
                ...
                case 'alquran':
                    ...
                    urls = `https://api.lolhuman.xyz/api/quran/${args[0]}?apikey=BayuPerkasa`
                ...
                case 'alquranaudio':
                    ...
                    surah = args[0]
                    ini_buffer = await getBuffer(`https://api.lolhuman.xyz/api/quran/audio/${surah}?apikey=BayuPerkasa`)
                    await rifqikhanif.sendMessage(from, ini_buffer, audio, { quoted: freply, mimetype: Mimetype.mp4Audio })
                    break
                case 'asmaulhusna':
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/asmaulhusna?apikey=BayuPerkasa`)
                    get_result = get_result.result
                ...
                case 'kisahnabi':
                    ...
                    query = args.join(" ")
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/kisahnabi/${query}?apikey=BayuPerkasa`)
                    get_result = get_result.result
                ...
                case 'jadwalsholat':
                    ...
                    daerah = args.join(" ")
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/sholat/${daerah}?apikey=BayuPerkasa`)
                    get_result = get_result.result
                    ...
                    break
      case 'rules':
             rifqikhanif.sendMessage(from, rulesBot(prefix), MessageType.text, {quoted: freply})
             break
               ...
               case 'slow':
					...
					media = await rifqikhanif.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp3')
					exec(`ffmpeg -i ${media} -filter:a "atempo=0.7,asetrate=44100" ${ran}`, (err, stderr, stdout) => {
						fs.unlinkSync(media)
						if (err) return reply('Error!')
						hah = fs.readFileSync(ran)
						rifqikhanif.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: freply})
						fs.unlinkSync(ran)
					})
					break
				case 'stickerlist':
...
break
			...
			case 'gets':
			   if (args.length < 1) return reply('Masukan nama yang terdaftar di list sticker')
				...
				rifqikhanif.sendMessage(from, result, sticker)
				break
           case 'liststicker':
				teks = '*Sticker List :*\n\n'
				for (let awokwkwk of setiker) {
			...
			case 'addvideo':
				...
				rifqikhanif.sendMessage(from, `Sukses Menambahkan Video\nCek dengan cara ${prefix}listvideo`, MessageType.text, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "_SelfBot OnlyDev City_", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JStw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/odc.jpeg')} }} }) 
				break
			case 'getvideo':
			   if (args.length < 1) return reply('Masukan nama yang terdaftar di list video')
				namastc = body.slice(10)
				buffer = fs.readFileSync(`./src/video/${namastc}.mp4`)
				rifqikhanif.sendMessage(from, buffer, video, { mimetype: 'video/mp4', quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "_SelfBot OnlyDev City_", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JStw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/odc.jpeg')} }} }) 
				break
			case 'listvideo':
			case 'videolist':
				teks = '*List Video :*\n\n'
...
[ ${args[0]} ] Ketik Y/N untuk menerima atau menolak permainan`
               ...
               rifqikhanif.sendMessage(from, starGame, text, {quoted: freply, contextInfo: { mentionedJid: [sender, args[0].replace("@", "") + "@s.whatsapp.net"],}})
...
Ket : Ketik /resetgame , Untuk Mereset Permainan Yg Ada Di Grup!`, text, {contextInfo: {mentionedJid: [player2]}})
     ...
     case 't':
       titid =  `SILAHKAN PILIH DI BAWAH UNTUK MELANJUTKAN`
                   ...
                   sendButMessage(from, titid, `crated rifqikhanif ads`, [
          {
            buttonId: `Y`,
            buttonText: {
              displayText: `Y`,
       ...
       case 'family100':
              ...
              anu = await axios.get(`http://api.lolhuman.xyz/api/tebak/family100?apikey=BayuPerkasa`)
       ...
       case 'tebakanime':
              ...
              get_result = await fetchJson(`https://lolhuman.herokuapp.com/api/tebakchara?apikey=BayuPerkasa`)
              get_result = get_result.result
              ini_image = get_result.image
              jawaban = get_result.name
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              ini_buffer = await getBuffer(ini_image)
              rifqikhanif.sendMessage(from, ini_buffer, image, { quoted: freply, caption: '*+* ```Tebak Anime```\n\n• *Petunjuk* :'+kisi_kisi+'\n• *Waktu* : 30s' }).then(() => {
       ...
       case 'tebaklagu':
              ...
              reply('*+* ```Tebak Lagu```\n\n• *Petunjuk* :'+kisi_kisi+'\n• *Waktu* : 30s')
              rifqikhanif.sendMessage(from, ini_buffer, audio, {quoted: freply}).then(() => {
       ...
       case 'tebaktebakan':
              ...
              rifqikhanif.sendMessage(from, '*+* ```Tebak Tebakan```\n\n• *soal* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: freply}).then(() => {
       ...
       case 'kuismath':
              ...
              jawaban = get_result.jawaban
              ini_buffer = await getBuffer(ini_image)
              rifqikhanif.sendMessage(from, ini_buffer, image, { quoted: freply, caption: '*+* ```Kuis Matematika```\n\nSilahkan jawab soal berikut ini\n\n• *Waktu* : 50s' }).then(() => {
       ...
       case 'tebakgambar':
            ...
            rifqikhanif.sendMessage(from, buff, image, { quoted: freply, caption: 'Silahkan jawab soal berikut ini\n\nPetunjuk :'+kisi_kisi+'\nWaktu : 30s' }).then(() => {
       ...
       case 'siapaaku':
              ...
              get_result = await fetchJson(`https://lolhuman.herokuapp.com/api/tebak/siapaaku?apikey=BayuPerkasa`)
              get_result = get_result.result
              jawaban = get_result.answer
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.question
              rifqikhanif.sendMessage(from, '*+* ```Tebak Siapakah Aku```\n\n• *soal* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: freply}).then(() => {
       ...
       case 'tebakkata':
              ...
              get_result = await fetchJson(`https://lolhuman.herokuapp.com/api/tebak/kata?apikey=BayuPerkasa`)
              get_result = get_result.result
              jawaban = get_result.jawaban
              pertanyaan = get_result.pertanyaan
              rifqikhanif.sendMessage(from, '*+* ```Tebak Kata```\n\n• *Soal* :'+pertanyaan+'\n• *Waktu :* 30s', text, { quoted: freply}).then(() => {
       ...
       case 'tebaklirik':
              ...
              get_result = await fetchJson(`https://lolhuman.herokuapp.com/api/tebak/lirik?apikey=BayuPerkasa`)
              get_result = get_result.result
              jawaban = get_result.answer
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.question
              rifqikhanif.sendMessage(from, '*+* ```Tebak Lirik```\n\n• *Soal* :'+pertanyaan+'\n• *Kisi²* :'+kisi_kisi, text, { quoted: freply}).then(() => {
      ...
      case 'tebakjenaka':
              ...
              get_result = await fetchJson(`https://lolhuman.herokuapp.com/api/tebak/jenaka?apikey=BayuPerkasa`)
              get_result = get_result.result
              jawaban = get_result.answer
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.question
              rifqikhanif.sendMessage(from, '*+* ```Tebak Jenaka```\n\n• *Soal* :'+pertanyaan+'\n• *Kisi²* :'+kisi_kisi, text, { quoted: freply}).then(() => {
       ...
       case 'kimiakuis':
              ...
              get_result = await fetchJson(`https://lolhuman.herokuapp.com/api/tebak/unsurkimia?apikey=BayuPerkasa`)
              get_result = get_result.result
              jawaban = get_result.lambang
              pertanyaan = get_result.nama
              rifqikhanif.sendMessage(from, '*+* ```Tebak Kimia```\n\n• *Soal* :'+pertanyaan+'\n• *Waktu :* 30s', text, { quoted: freply}).then(() => {
       ...
       case 'tebakbendera':
              ...
              get_result = await fetchJson(`https://lolhuman.herokuapp.com/api/tebak/bendera?apikey=BayuPerkasa`)
              get_result = get_result.result
              jawaban = get_result.name
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.flag
              rifqikhanif.sendMessage(from, '*+* ```Tebak Bendera```\n\n• *Bendera* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: freply}).then(() => {
       ...
       case 'susunkata':
              ...
              get_result = await fetchJson(`https://lolhuman.herokuapp.com/api/tebak/susunkata?apikey=BayuPerkasa`)
              get_result = get_result.result
              jawaban = get_result.jawaban
              pertanyaan = get_result.pertanyaan
              rifqikhanif.sendMessage(from, '*+* ```Susun Kata```\n\n• *Soal* :'+pertanyaan+'\n• *Waktu :* 30s', text, { quoted: freply}).then(() => {
       ...
       case 'asahotak':
              ...
              get_result = await fetchJson(`https://lolhuman.herokuapp.com/api/tebak/asahotak?apikey=BayuPerkasa`)
              get_result = get_result.result
              jawaban = get_result.jawaban
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.pertanyaan
              rifqikhanif.sendMessage(from, '*+* ```Asah Otak```\n\n• *soal* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: freply}).then(() => {
       ...
       case 'caklontong':
              ...
              get_result = await fetchJson(`https://lolhuman.herokuapp.com/api/tebak/caklontong2?apikey=BayuPerkasa`)
              get_result = get_result.result
              jawaban = get_result.answer
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.question
              rifqikhanif.sendMessage(from, '*+* ```Caklontong```\n\n• *soal* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: freply}).then(() => {
...
const serialUser = createSerial(18)
	         try {
								ppimg = await rifqikhanif.getProfilePicture(`${sender.split('@')[0]}@c.us`)
								} catch {
...
*「 NIGHTBOT 」*`
         IkyAds = await getBuffer(`http://hadi-api.herokuapp.com/api/card/verify?nama=${encodeURI(pushname)}&member=${_registered.length}&seri=${serialUser}&pp=${ppimg}&bg=${ppimg}`)
             buttons = [{buttonId: `!menu`,buttonText:{displayText: `🏷️MENU`},type:1}]
              imageMsg = (await rifqikhanif.prepareMessageMedia(IkyAds, "imageMessage", { thumbnail: IkyAds, })).imageMessage
...
case 'sfire':
{
                ...
                if (isImage || isQuotedImage) {
                    ...
                    let yoooo = await rifqikhanif.downloadAndSaveMediaMessage(encmedia)
...
`
  buttons = [{buttonId: `${prefix}owner`,buttonText:{displayText: `OWNER`},type:1},{buttonId: `${prefix}payment`,buttonText:{displayText: `PAYMENT`},type:1}]
      ...
      case 'payment':
      list = await rifqikhanif.prepareMessageFromContent(from, {
    "listMessage": {
      "title": "PAYMENT",
      "description": "CLICK",
      "buttonText": "",
  ...
  rifqikhanif.relayWAMessage(list, {waitForAck: true})
...
//------------------< Downloader/Search/Anime >-------------------
       ...
       case 'soundcloud':
              ...
              reply(mess.wait)
              anu = await fetchJson(`https://api.lolhuman.xyz/api/soundcloud?apikey=BayuPerkasa&url=${q}`)
...
_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
        ...
        case 'google':
              ...
              vars +=  `\n═════════════════\n\n*Judul:* ${results[i].title}\n\n*Deskripsi:* ${results[i].snippet}\n\n*Link:* ${results[i].link}\n\n`
               ...
               rifqikhanif.sendMessage(from, ss, image, {thumbnail: Buffer.alloc(0), caption: vars, quoted : kyy})
               }).catch(e => {
               console.log(e)
               reply(`${e}`)
})
...
if (err) return reply(`${err}`)
rifqikhanif.sendMessage(from,{url:'./'+kyyyy},audio,{mimetype:'audio/mpeg'})
})
       ...
       case 'tiktoknowm':
              ...
              imageMsg = ( await rifqikhanif.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
  ...
  case 'xnxxsearch':
              ...
              /////////////////////////////////////////////////////////////////////////////////////////////////////////////////if (!isRegister) return reply(`You are not verified\n\nReply this chat and send bot password\n\nHint : \nPassword contains 4 digit number\nCheck password at: https://rifqikhanif-chan02.github.io`)
      ...
      case 'tiktokaudio':
             ...
             ini_link = args[0]
             get_audio = await getBuffer(`https://api.lolhuman.xyz/api/tiktokmusic?apikey=BayuPerkasa&url=${ini_link}`)
             rifqikhanif.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, quoted: freply })
             break
      case 'fbdl':
      case 'fb':
      case 'facebok':
					...
					rifqikhanif.sendMessage(from, buff, image, {quoted: freply, caption: teks})
					break
      case 'twitter':
      ...
      case 'ssweb':
             ...
             await rifqikhanif.sendMessage(from, ini_buffer, image, { quoted: freply })
             break
       case 'nhentaipdf':
             if (!isPremium) return sendButMessage (from, prem1, prem2, prem3, { quoted: freply})
             ...
             try {
             henid = args[0]
             get_result = await fetchJson(`https://api.lolhuman.xyz/api/nhentai/${henid}?apikey=BayuPerkasa`)
             get_result = get_result.result
...
🐤 Character : ${get_info.characters.join(", ")}\n`
             ...
             rifqikhanif.sendMessage(from, ini_image, image, { caption: teks, quoted: freply, thumbnail: Buffer.alloc(0) })
             reply(mess.wait)
             anu = await fetchJson(`https://api.lolhuman.xyz/api/nhentaipdf/${henid}?apikey=BayuPerkasa`)
             pdf = await getBuffer(anu.result)
             rifqikhanif.sendMessage(from, pdf, document, { quoted: freply, mimetype: Mimetype.pdf, filename: `${get_result.title_romaji}.pdf`, thumbnail: ini_image })
             } catch (e) {
             console.log(e)
             reply(String(e))
}
       ...
       case 'manga':
              ...
              reply(mess.wait)
              query = args.join(" ")
              get_result = await fetchJson(`https://api.lolhuman.xyz/api/manga?apikey=BayuPerkasa&query=${query}`)
              get_result = get_result.result
       ...
       case 'doujindesu':
             ...
             rifqikhanif.sendMessage(from, buffer, image, {caption: xixixai, quoted: freply})
             } catch (e) {
             console.log(e)
             reply(String(e))
}
      ...
      case 'anime':
             ...
             reply(mess.wait)
             query = args.join(" ")
             get_result = await fetchJson(`https://api.lolhuman.xyz/api/anime?apikey=BayuPerkasa&query=${query}`)
             get_result = get_result.result
             ini_txt = `Id : ${get_result.id}\n`
             ini_txt += `Id MAL : ${get_result.idMal}\n`
             ini_txt += `Title : ${get_result.title.romaji}\n`
      ...
      case 'kusonime':
             ...
             reply(mess.wait)
             query = args.join(" ")
             get_result = await fetchJson(`https://api.lolhuman.xyz/api/kusonimesearch?apikey=BayuPerkasa&query=${query}`)
             get_result = get_result.result
             ini_txt = `Title : ${get_result.title}\n`
             ...
             await rifqikhanif.sendMessage(from, ini_buffer, image, { quoted: freply, caption: ini_txt })
             break
       case 'otakudesu':
              if (args.length == 0) return reply(`Example: ${prefix + command} Gotoubun No Hanayome`)
              reply(mess.wait)
              query = args.join(" ")
              get_result = await fetchJson(`https://api.lolhuman.xyz/api/otakudesusearch?apikey=BayuPerkasa&query=${query}`)
              get_result = get_result.result
              ini_txt = `Title : ${get_result.title}\n`
              ini_txt += `Japanese : ${get_result.japanese}\n`
       ...
       case 'nekopoi':
              ...
              ini_url = args[0]
              get_result = await fetchJson(`https://api.lolhuman.xyz/api/nekopoi?apikey=BayuPerkasa&url=${ini_url}`)
              get_result = get_result.result
       ...
       case 'nekopoisearch':
              ...
              query = args.join(" ")
              get_result = await fetchJson(`https://api.lolhuman.xyz/api/nekopoisearch?apikey=BayuPerkasa&query=${query}`)
              get_result = get_result.result
              ini_txt = ""
              for (var x of get_result) {
              ini_txt += `\`\`\`🐥 Title : ${x.title}\`\`\`\n`
       ...
       case 'storyanime':
              ...
              reply(mess.wait)
              anu = await fetchJson(`https://lolhuman.herokuapp.com/api/storynime?apikey=BayuPerkasa`)
              buffer = await getBuffer(anu.result)
              rifqikhanif.sendMessage(from, buffer, video, { quoted: freply })
              break
       case 'nekopoi3d':
       case '3dnekopoi':
       case '3dnekopoilast':
...
*[Wait] Tunggu Sebentar..*`
				...
				rifqikhanif.sendMessage(from, buffer1, video, {mimetype: 'video/mp4', filename: `${anu.result.video}.mp4`, quoted:freply, caption: 'Nih Gan'})
					break  
...
imageMessage = (await rifqikhanif.prepareMessageMedia({url:res.all[0].image},'imageMessage',{thumbnail:Buffer.alloc(0)})).imageMessage
...
break
          ...
          case 'lirik':
                    ...
                    query = args.join(" ")
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/lirik?apikey=BayuPerkasa&query=${query}`)
                    reply(get_result.result)
         ...
         case 'pinterest':
		   if (args.length == 0) return reply(`Example: ${prefix + command} loli`)
                    query = args.join(" ")
                 reply (mess.wait)
                    ini_url = await fetchJson(`http://api.lolhuman.xyz/api/pinterest?apikey=BayuPerkasa&query=${query}`)
...
\`\`\`🐥 Link : ${get_data[i].url}\`\`\``
              ...
              rifqikhanif.sendMessage(from, ini_buffer, image, { quoted: freply, caption: teks })
              } catch {
...
\`\`\`🔗 Link : ${i.url}\`\`\``
       ...
       case 'tourl':
               ...
               owgi = await rifqikhanif.downloadMediaMessage(boij)
       ...
       case 'img2url':
               ...
               var media = await  rifqikhanif.downloadAndSaveMediaMessage(encmedia)       
               imgbb('39d895963468b814fad0514bd28787e2', media)
              .then(data => {
               var caps = `*_IMAGE TO URL_*\n\n*~>  ID :* ${data.id}\n*~>  MimeType :* ${data.image.mime}\n*~>  Extension :* ${data.image.extension}\n*~>  URL :* ${data.display_url}`
               ibb = fs.readFileSync(media)
               rifqikhanif.sendMessage(from, ibb, image, { quoted: freply, caption: caps})
})
              .catch(err => {
               throw err
})
        ...
        case 'tulis':
               ...
               rifqikhanif.sendMessage(from, buff, image, {quoted: freply, caption: mess.success}).catch(e => {
...
//------------------< Level >-------------------
      case 'level': 
              ...
              try {
              profilePic = await rifqikhanif.getProfilePicture(sender)
              } catch {
              profilePic = errorImg
}
              buffer = await getBuffer(`https://lolhuman.herokuapp.com/api/rank?apikey=BayuPerkasa&img=${profilePic}&background=https://telegra.ph/file/443b6600636aed1d94acd.jpg&username=${encodeURI(pushname)}&level=${userLevel}&ranking=${Number(userRank)}&currxp=${userXp}&xpneed=${requiredXp}`)
              teks = `*「 LEVEL 」*\n\n➸ *Nama :* ${pushname}\n➸ *Xp :* ${userXp} / ${requiredXp}\n➸ *Level :* ${userLevel}\n➸ *Role*: *${role}*\n\n*Note : Kumpulin Xp Jika Ingin Menaikkan Level*`
              rifqikhanif.sendMessage(from, buffer, image, { caption: teks, quoted: freply})
              break
...
//------------------< Stalk >-------------------
      case 'stalkgithub':
      case 'githubstalk':
              if (args.length == 0) return reply(`Example: ${prefix + command} rifqikhanif-chan02`)
              reply(mess.wait)
              username = args[0]
              ini_result = await fetchJson(`https://api.lolhuman.xyz/api/github/${username}?apikey=BayuPerkasa`)
...
\`\`\`🐥 Link : ${ini_result.url}\`\`\`
`
             rifqikhanif.sendMessage(from, ini_buffer, image, { caption: ini_txt, thumbnail: Buffer.alloc(0) })
             break
      case 'stalkig':
      case 'igstalk':
             if (args.length == 0) return reply(`Example: ${prefix + command} rifqikhanif.chan26`)
             reply(mess.wait)
             username = args[0]
             ini_result = await fetchJson(`https://api.lolhuman.xyz/api/stalkig/${username}?apikey=BayuPerkasa`)
             ini_result = ini_result.result
...
`
      ...
      case 'tiktokstalk':
             ...
             reply(mess.wait)
             stalk_toktok = args[0]
             get_result = await fetchJson(`http://lolhuman.herokuapp.com/api/stalktiktok/${stalk_toktok}?apikey=BayuPerkasa`)
             get_result = get_result.result
...
`
              rifqikhanif.sendMessage(from, pp_tt, image, { quoted: freply, caption: ini_txt, thumbnail: Buffer.alloc(0) })
              break
       case 'iguser':
              try {
...
//------------------< Sticker/Tools >-------------------
      ...
      case 'doge':
       ...
       case 'telestiker':
              ...
              reply(mess.wait)
              ini_url = await fetchJson(`https://api.lolhuman.xyz/api/telestick?apikey=BayuPerkasa&url=${args[0]}`)
              ini_sticker = ini_url.result.sticker
              reply('Sending '+ ini_sticker.length +' stickers...')
              for (sticker_ in ini_sticker) {
              ini_buffer = await getBuffer(ini_sticker[sticker_])
              rifqikhanif.sendMessage(from, ini_buffer, sticker, {})
}
              break
       case 'semoji':
       case 'emoji':
              ...
              ini_buffer = await getBuffer(`https://api.lolhuman.xyz/api/smoji/${emoji}?apikey=BayuPerkasa`)
              await rifqikhanif.sendMessage(from, ini_buffer, sticker, { quoted: freply })
              break
case 'ttp':
...
break
       case 'attp':
              if (args.length == 0) return reply(`Example: ${prefix + command} rifqikhanif`)
              buffer = await getBuffer(`https://api.xteam.xyz/attp?file&text=${encodeURI(q)}`)
              rifqikhanif.sendMessage(from, buffer, sticker, { quoted: freply })
              break
              case 'tg': 
...
break
       ...
       case 'triggered':
                ...
                if (isImage || isQuotedImage) {
                    let encmedia = isQuotedImage ? JSON.parse(JSON.stringify(kyy).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : kyy
                    let media = await rifqikhanif.downloadAndSaveMediaMessage(encmedia)
                ...
                exec(`webpmux -set exif ./sticker/data.exif ./sticker/triggered.webp -o ./sticker/triggered.webp`, async (error) => {
                    if (error) return reply(mess.error.api)
                    rifqikhanif.sendMessage(from, fs.readFileSync(`./sticker/triggered.webp`), sticker, {quoted: freply})
           ...
           teks = args.join(' ')
                ...
                let ane = await getBuffer(`https://api.lolhuman.xyz/api/attp?apikey=BayuPerkasa&text=${mek}`)
                ...
                exec(`webpmux -set exif ./sticker/data.exif ./sticker/ttg.webp -o ./sticker/ttg.webp`, async (error) => {
                    if (error) return reply(mess.error.api)
                    rifqikhanif.sendSticker(from, fs.readFileSync(`./sticker/ttg.webp`), sticker, {quoted: freply})
                    limitAdd(sender, limit)
       ...
       case 'gifstiker':
            ...
            const media = await rifqikhanif.downloadAndSaveMediaMessage(encmedia)
                ran = '666.webp'
                await ffmpeg(`./${media}`)
                .input(media)
                .on('start', function (cmd) {
                ...
                .on('end', function () {
                console.log('Finish')
                rifqikhanif.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: freply})
                 fs.unlinkSync(media)
                fs.unlinkSync(ran)
                })
                .addOutputOptions([`-vcodec`, `libwebp`, `-vf`, `scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
       ...
       case 'colong':
              ...
              require('./lib/fetch.js').modStick(media, rifqikhanif, kyy, from)
              break
       case 'delwm':
              if (!isQuotedSticker) return reply('Stiker aja om')
              encmedia = JSON.parse(JSON.stringify(kyy).replace('quotedM','m')).message.extendedTextMessage.contextInfo
       ...
       case 'swm':
             ...
             .on('error', (e) => {
              console.log(e)
              rifqikhanif.sendMessage(from, 'Terjadi kesalahan', 'conversation', { quoted: freply })
              fs.unlinkSync(media)
})
             .on('end', () => {
            _out = getRandom('.webp')
             ...
             .on('exit', () => {
              rifqikhanif.sendMessage(from, fs.readFileSync(_out),'stickerMessage', { quoted: freply })
              fs.unlinkSync(out)
              fs.unlinkSync(_out)
              fs.unlinkSync(media)
})
      ...
      case 'toimg':
              ...
              rifqikhanif.sendMessage(from, buffer, image, {quoted: freply, caption: 'Nih'})
              fs.unlinkSync(ran)
})
              break
       case 'smeme': 
...
break
       ...
       case 'memegen':                    
        ...
        case 'togif':
               ...
               mediaaa = await rifqikhanif.downloadAndSaveMediaMessage(encmediaaa)
               a = await webp2gifFile(mediaaa)
               mp4 = await getBuffer(a.result)
               rifqikhanif.sendMessage(from, mp4, video, {mimetype: 'video/gif', quoted: freply, caption: mess.success})
        ...
        case 'tovideo':
               ...
               a = await webp2gifFile(mediaaa)
               mp4 = await getBuffer(a.result)
               rifqikhanif.sendMessage(from, mp4, video, {mimetype: 'video/mp4', quoted: freply, caption: mess.success})
               fs.unlinkSync(mediaaa)
               } else {
               reply(mess.wrongFormat)
}
...
buffer = fs.readFileSync(ran)
rifqikhanif.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4'})
fs.unlinkSync(ran)
})
break
...
//------------------< Ingfo Bot >-------------------
      case 'runtime':
       ...
       case 'sc': 
       case 'src':
              textImg(`tanya owner,nih sebelum kehabisan! https://wa.me/17155448800?text=bang+minta+sc+`)
              break
      case 'ping':
      case 'speed':
              timestampe = speed();
...
\`\`\`🐥 Wa Version: ${rifqikhanif.user.phone.wa_version}\`\`\`
\`\`\`🐥 Os Version: ${rifqikhanif.user.phone.os_version}\`\`\`
\`\`\`🐥 Device Manufacturer: ${rifqikhanif.user.phone.device_manufacturer}\`\`\`
\`\`\`🐥 Device Model: ${rifqikhanif.user.phone.device_model}\`\`\`
\`\`\`🐥 Os Build Number: ${rifqikhanif.user.phone.os_build_number}\`\`\``
             reply(teks)
             break  
//------------------< Owner >-------------------
      case 'addupdate':
      ...
      case 'join': 
             ...
             fak = rifqikhanif.query({ json: ['action', 'invite', link],
             expect200: true })
      ...
      case 'broadcast':
             ...
             anu = await rifqikhanif.chats.all()
             if (isMedia && !kyy.message.videoMessage || isQuotedImage) {
             const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(kyy).replace('quotedM','m')).message.extendedTextMessage.contextInfo : kyy
             bc = await rifqikhanif.downloadMediaMessage(encmedia)
             for (let _ of anu) {
             rifqikhanif.sendMessage(_.jid, bc, image, {quoted:freply,caption: `*「 NIGHTBOT NEWS 」*\n\n${body.slice(4)}`})
}
...
rifqikhanif.sendMessage(_.jid, 
      ...
      case 'clearall':
             if (!isOwner) return  reply(mess.only.owner)
             anu = await rifqikhanif.chats.all()
             rifqikhanif.setMaxListeners(25)
             for (let _ of anu) {
             rifqikhanif.deleteChat(_.jid)
}
      ...
      case 'leaveall':
             if (!isOwner) return  reply(mess.only.owner)
             let totalgroup = rifqikhanif.chats.array.filter(u => u.jid.endsWith('@g.us')).map(u => u.jid)
             for (let id of totalgroup) {
             sendMess(id, 'Byee', null)
             await sleep(3000)
             rifqikhanif.groupLeave(id)
}
             break
//------------------< G R U P >-------------------
      case 'kick':
      ...
      case 'add':
             ...
             rifqikhanif.groupAdd(from, [entah])
             } else {
      ...
      case 'promote':
             ...
             rifqikhanif.groupMakeAdmin(from, mems_ids)
             } else {
             rifqikhanif.groupMakeAdmin(from, entah)
}
             } else {
             entah = kyy.message.extendedTextMessage.contextInfo.participant
             rifqikhanif.groupMakeAdmin(from, [entah])
      ...
      case 'demote':
             ...
             rifqikhanif.groupDemoteAdmin(from, mems_ids)
             } else {
             rifqikhanif.groupDemoteAdmin(from, [entah[0]])
}
             } else {
             entah = kyy.message.extendedTextMessage.contextInfo.participant
             rifqikhanif.groupDemoteAdmin(from, [entah])
}
             break
       case 'setgrupname':
              if (!isGroup) return reply(mess.only.group)
              ...
              rifqikhanif.groupUpdateSubject(from, q)
       ...
       case 'setdesc':
              ...
              rifqikhanif.groupUpdateDescription(from, q)
             .then((res) => reply(jsonformat(res)))
             .catch((err) => reply(jsonformat(err)))
              break
       case 'setppgrup':
              ...
              if (isQuotedImage) {
              let encmedia = isQuotedImage ? JSON.parse(JSON.stringify(kyy).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : kyy
              let media = await rifqikhanif.downloadMediaMessage(encmedia)
              rifqikhanif.updateProfilePicture(from, media)
             .then((res) => reply(jsonformat(res)))
             .catch((err) => reply(jsonformat(err)))
              } else {
       ...
       case 'profile':
              ...
              rifqikhanif.updatePresence(from, Presence.composing)
              try {
              profil = await rifqikhanif.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
              } catch {
              profil = errorImg
}
              thu = await rifqikhanif.getStatus(`${sender.split('@')[0]}@s.whatsapp.net`, MessageType.text)
              ...
              rifqikhanif.sendMessage(from, buff, image, {quoted: freply, caption: profile})
              break
       case 'afk': 
              if (!isGroup) return reply(mess.only.group)
       ...
       case 'groupinfo':
              if (!isGroup) return reply(mess.only.group)
              try {
              var pic = await rifqikhanif.getProfilePicture(from)
              } catch {
              var pic = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
}
              let ingfo = `*G R O U P I N F O*\n\n*Name :* ${groupName}\n*ID Grup :* ${from}\n*Dibuat :* ${moment(`${groupMetadata.creation}` * 1000).tz('Asia/Jakarta').format('DD/MM/YYYY HH:mm:ss')}\n*Owner Grup :* @${groupMetadata.owner.split('@')[0]}\n*Jumlah Admin :* ${groupAdmins.length}\n*Jumlah Peserta :* ${groupMembers.length}\n*Welcome :* ${isWelkom ? 'Aktif' : 'Mati'}\n*AntiLink :* ${isAntiLink ? 'Aktif' : 'Mati'}\n*Desc :* \n${groupMetadata.desc}`
              rifqikhanif.sendMessage(from, await getBuffer(pic), image, {quoted: freply, caption: ingfo, contextInfo: {"mentionedJid": [groupMetadata.owner.replace('@c.us', '@s.whatsapp.net')]}})
              break
       case 'tagall':
              if (!isGroup) return reply(mess.only.group)
              let arr = [];
       ...
       case 'here':                
             ...
             let online = [...Object.keys(rifqikhanif.chats.get(ido).presences), rifqikhanif.user.jid]
             rifqikhanif.sendMessage(from, 'List Online:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, { quoted: freply, contextInfo: { mentionedJid: online }})
             } catch (e) {
             reply(`${e}`)
}
             break
      ...
      case 'sider':
             if(!isGroup) return reply(mess.only.group)
             try {
             infom = await rifqikhanif.messageInfo(from, kyy.message.extendedTextMessage.contextInfo.stanzaId)
             tagg = []
             teks = `*• Dibaca oleh:*\n\n`
...
//------------------< Fun >-------------------
       ...
       case 'toxic':
        ...
        case 'citacita':
              ...
              rifqikhanif.sendMessage(from, cita2, audio,{mimetype: 'audio/mp4', ptt:true, quoted: freply})
              break
       case 'apakah':
              apakah = body.slice(1)
              const apa =['Iya','Tidak','Bisa Jadi','Coba Ulangi']
              const kah = apa[Math.floor(Math.random() * apa.length)]
              rifqikhanif.sendMessage(from, '*Pertanyaan :* '+apakah+'\n*Jawaban :* '+ kah, text, { quoted: freply })
              break
       case 'rate':
       case 'nilai':
              rate = body.slice(1)
              ...
              rifqikhanif.sendMessage(from, '*Pertanyaan :* '+rate+'\n*Jawaban :* '+ te+'%', text, { quoted: freply })
              break
      case 'bay':
      ...
      break
       ...
       case 'cekganteng':
              ...
              rifqikhanif.sendMessage(from, '*Pertanyaan :* '+ganteng+'\n*Jawaban :* '+ teng+'%', text, { quoted: freply })
              break
       case 'cantikcek':
       case 'cekcantik':
              cantik = body.slice(1)
              ...
              rifqikhanif.sendMessage(from, '*Pertanyaan :* '+cantik+'\n*Jawaban :* '+ tik+'%', text, { quoted: freply })
              break
       case 'cekwatak':
              var namao = pushname
              var prfx = await rifqikhanif.getProfilePicture(sender)
       ...
       case 'truth':
              ...
              rifqikhanif.sendMessage(from, truteh, image, { caption: '*Truth*\n\n'+ ttrth, quoted: freply })
              break
       case 'dare':
...
//------------------< Lainnya >-------------------
        case 'getpp':
               if (kyy.message.extendedTextMessage === null || kyy.message.extendedTextMessage === undefined) {
               linkpp = await rifqikhanif.getProfilePicture(from) || "https://telegra.ph/file/40151a65238ba2643152d.jpg"
               buffer = await getBuffer(linkpp)
               rifqikhanif.sendMessage(from, buffer, image, {caption: "Nih", quoted: freply })
               ...
               linkpp = await rifqikhanif.getProfilePicture(mberr) || "https://telegra.ph/file/40151a65238ba2643152d.jpg"
               buffer = await getBuffer(linkpp)
               rifqikhanif.sendMessage(from, buffer, image, { quoted: freply, caption: `Profile Picture of @${mberr.split("@")[0]}`, contextInfo: { "mentionedJid": [mberr] }})
        ...
        case 'delete': // MR.CYSER
               ...
               rifqikhanif.deleteMessage(from, {id: kyy.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true})
               } catch (e){
               reply('Reply chat bot')
}
               break
...
│
└──「 *${botName}* 」`
             rifqikhanif.sendMessage(from, await getBuffer(urlinfo), image, {quoted: freply, caption: thankslort })
             break
case 'media':
...
case 'caripesan':  //by ANU TEAM
             ...
             reply(mess.wait)
             cok = await rifqikhanif.searchMessages(`${ve}`, from, batas,1) 
             if (cok.messages.length < 2) return reply('Tidak Ditemukan Pesan') 
             if (cok.messages.length < parseInt(batas)) reply(`Hanya Ditemukan ${cok.messages.length - 1} Pesan`)
             for (i=1;i < cok.messages.length;i++) {
             if (cok.messages[i].message) {
             rifqikhanif.sendMessage(from, `Ditemukan!`, text, {sendEphemeral: true, quoted: cok.messages[i]}) 
}
}
             } catch (e) {
       ...
       case 'bugreport':
              ...
              rifqikhanif.sendMessage('6288227606701@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'readall':
              totalchat.map( async ({ jid }) => {
              await rifqikhanif.chatRead(jid)
})
...
//------------------< Enable/Disable >-------------------
      ...
      case 'grup':
             ...
             if (args[0].toLowerCase() === 'enable'){
             rifqikhanif.groupSettingChange(from, "announcement", false)
            .then((res) => reply(jsonformat(res)))
            .catch((err) => reply(jsonformat(err)))
             } else if (args[0].toLowerCase() === 'disable'){
             rifqikhanif.groupSettingChange(from, "announcement", true)
            .then((res) => reply(jsonformat(res)))
            .catch((err) => reply(jsonformat(err)))
...
//------------------< Menunya Bang:v >-------------------
      case 'infoig':
             reply(`GAK MAIN IGEE`)
             break
      case 'rifqikhanifgroup':
             reply('dah gw bubarin banh')
             break
...
• Pemenangnya adalah [ @${winR} ] `
	   rifqikhanif.sendMessage(from, starGame, text, {quoted: freply, contextInfo: { mentionedJid: [winR + "@s.whatsapp.net", gelutSkuy.Z + "@s.whatsapp.net", gelutSkuy.Y + "@s.whatsapp.net",]}})
		fs.unlinkSync("./media/" + from + ".json");
	} else if (sender == `${gelutSkuy.Y}@s.whatsapp.net` &&  budy.toLowerCase() == 'n') {
		rifqikhanif.sendMessage(from, `👑 Game Gelud Rejected 🤙🏻
...
Giliran = @${tty.player1.split('@')[0]}
  ...
  rifqikhanif.sendMessage(from, ucapan, text, {quoted: freply, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
  }
...
naa = ky_ttt.filter(toek => !toek.id.includes(from)) 
ky_ttt = naa
rifqikhanif.sendMessage(from, `Yahh @${tty.player2.split('@')[0]} Menolak:(`,text,{quoted:kyy,contextInfo:{mentionedJid:[tty.player2]}})
}
}
...
${ttt}`
rifqikhanif.sendMessage(from, ucapan1, text, {quoted: freply, contextInfo:{mentionedJid: [tty.player1]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
${ttt}`
rifqikhanif.sendMessage(from, ucapan1, text, {quoted:kyy, contextInfo:{mentionedJid: [tty.player2]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
